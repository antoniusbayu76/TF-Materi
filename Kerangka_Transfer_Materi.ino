...
void UltrasSamping(){
    ...
    //Berikan fungsi untuk ultrasonik samping
    //Meliputi
    //Inisiasi Trigger, Echo, pengaturan distance
    ...
    if (distance <= 75) {
      ...
      //Hampir nabrak, bantu aktuator belok ke kiri
    } 
  
    if (distance2 <= 75) {
      setColor(255, 255, 0);  // Yellow
      //Hampir nabrak, bantu aktuator belok ke kanan
    }
    //Apabila memungkinkan untuk diperbaiki/direvisi
    //monggo bebas
}
...
void UltrasDepan(){
    ...
    Serial.println("UD");    
    //Fungsi Ultrasonik depan & Distance
...
//6. Fungsi Pergantian Mode Idle dengan 1 button, biar
//ringkes wiringnya ya, jadi kalau bisa 1 button aja
int checkMode(int Counter) {
  //Bebas mau bereksperimen seperti apa metodenya
  //Mau makai cara dummy atau counter terserah
}

//7. Fungsi button naik
int SpeedUp(int buttonPushCounter){

  //Contoh ya buat Fungsi button naik
...
//Buat tau rentang dari remote, bisa dicoba dulu 
//fungsi remote di atas, make pulseIn, ditulis rentangnya
...
//Mode Autonomous menggunakan Remote
  while (ch5 < 1400 && ch6 > 1400) 
  {
    ...
    lcd.setCursor(0, 0);
    lcd.print("A");
    //Cari cara ya biar bisa keluar dari while loopnya :))
  }
//Mode Masuk Remote
  while (ch5 > 1400 && ch6 < 1400) 
  {
...
/* 2. Masuk mode autonomous menggunakan button*/
  while (//Kondisi button + Kondisi remote)
  {
    setColor(0, 0, 255);  // blue
    autonomous();
    //Ups ada yang perlu ditambahin sepertinya :))
}

  /* 3. Masuk mode idle menggunakan button*/
if (//Kondisi button + Kondisi remote)
  {
...
/*CATATAN = Bisa improve atau berinovasi sesuai dengan 
 * imajinasi masing-masing ya, ga harus saklek cara di 
 * atas, kalau mau nambahin atau ngurangin dipersilakan
 * Kalau mau tanya bisa WA :)) 
 * Mangat gaes
 */
